    ...
    def add(self):
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=execute_fetchone("Select * from supplier where invoice=?",(self.var_sup_invoice.get(),))
                if row!=None:
                    messagebox.showerror("Error","Invoice no. is already assigned",parent=self.root)
                else:
                    execute_update("insert into supplier(invoice,name,contact,desc) values(?,?,?,?)",(
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if row==None:
                    ...
                    execute_update("update supplier set name=?,contact=?,desc=? where invoice=?",(
                        self.var_name.get(),
                        self.var_contact.get(),
                        self.txt_desc.get('1.0',END),
                        self.var_sup_invoice.get(),
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        execute_update("delete from supplier where invoice=?",(self.var_sup_invoice.get(),))
